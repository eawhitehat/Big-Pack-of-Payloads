...
domain=$1
if [[ -z $domain ]]; then
	echo -e "$GREEN Usage: ./easyRecon.sh <domain.com>"
	exit 1
fi
...
echo "	"
echo -e "\e[5m	Installing / Updating Tools so take some time. \e[0m"
echo "	"
echo "	"


sudo apt install httprobe -y > /dev/null 2>&1
sudo apt install getallurls  > /dev/null 2>&1``````````````````````````````````````````````````````````````````````
sudo go install github.com/tomnomnom/waybackurls@latest  > /dev/nul 2>&1
sudo go install github.com/sensepost/gowitness@latest > /dev/nul 2>&1
